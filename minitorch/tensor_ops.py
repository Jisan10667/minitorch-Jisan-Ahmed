...
def tensor_zip(
    ...
    def _zip(
        ...
        for i in range(int(np.prod(out_shape))):
            ...
            out[out_pos] = fn(a_storage[a_pos], b_storage[b_pos])  # Q4: What two values?
    ...
    def _reduce(
        ...
        for i in range(int(np.prod(out_shape))):
            ...
            for j in range(a_shape[reduce_dim]):
                a_index[reduce_dim] = j  # Q5: What index value?
                a_pos = index_to_position(a_index[:len(a_shape)], a_strides)
                out[out_pos] = fn(out[out_pos], a_storage[a_pos])  # Q6: Accumulate how?
